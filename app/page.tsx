...
export default function LandingPage() {
  ...
  const getCost = (t: string, d: number) => {
    const base = t === 'pro' ? 10 : 25;
    const multiplier = d === 1 ? 1 : d === 3 ? 2.5 : 5;
  ...
  const handleUpgrade = async () => {
    ...
    if (error) {
      ...
      setKeyData({ email, tier, duration, cost, expiry: expiryDate.toLocaleDateString() });
    }
    setLoading(false);
  };
      ...
      <section className="max-w-md mx-auto bg-gray-900 p-8 rounded-2xl border border-gray-800">
        ...
        <div className="space-y-4">
          ...
          <div className="text-center py-2 text-xl font-semibold">
            Estimated Cost: ${getCost(tier, duration)}
          </div>

          <button 
            onClick={handleUpgrade}
            disabled={loading}
            className="w-full bg-gradient-to-r from-purple-600 to-blue-600 p-3 rounded-lg font-bold hover:opacity-90 transition"
          >
            ...
            <div className="bg-gray-800 p-8 rounded-xl shadow-2xl max-w-sm w-full text-center border border-blue-500">
              ...
              <div className="flex flex-col gap-3">
                <button 
                  ...
                  className="bg-green-600 p-2 rounded font-bold"
                >
                  Download .txt File
                </button>
                <button 
